  ...
  return (
    <div className="p-5 rounded-2xl glass-panel border-border shadow-md flex flex-col gap-4 bg-surface h-full">
      ...
      {history.length === 0 ? (
        ...
        <div className="relative flex flex-col pl-4 border-l border-border gap-5 mt-2">
          {history.map((job, idx) => {
            ...
            return (
              <div key={idx} className="relative group">
                {/* Timeline node dot */}
                <div
                  className={`absolute -left-[21px] top-1.5 w-2.5 h-2.5 rounded-full border border-background transition-all duration-300
                    ${
                      job.isCurrent
                        ? "bg-emerald-400 shadow-[0_0_8px_#34d399]"
                        : "bg-text-disabled group-hover:bg-blue-400"
                    }`}
                />

                {/* Job Card */}
                <div className="flex flex-col gap-1.5 p-3 rounded-xl bg-surface border border-border group-hover:border-border transition-all duration-150">
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <button
                      ...
                      className="p-1 rounded bg-surface hover:bg-surface-hover text-muted hover:text-primary transition-colors"
                  ...
                  <AnimatePresence>
                    {isExpanded && (
                      ...
                      >
                        <p className="text-[11px] text-muted leading-relaxed font-sans whitespace-pre-line">
