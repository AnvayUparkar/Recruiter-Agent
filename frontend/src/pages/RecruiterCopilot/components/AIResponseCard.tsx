...
const renderMarkdown = (text: string) => {
  ...
  lines.forEach((line, i) => {
    ...
    if (line.startsWith("### ")) {
      elements.push(
        <h3 key={i} className="text-sm font-bold text-slate-100 mt-3 mb-1">
          {line.slice(4)}
        </h3>
      );
    }
    // h2
      ...
      elements.push(
        <h2 key={i} className="text-base font-bold text-slate-50 mt-4 mb-1">
          {line.slice(3)}
        </h2>
      );
    }
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="rounded-2xl rounded-tl-sm border border-white/10 bg-slate-900/70 backdrop-blur-xl overflow-hidden">
          {/* Content */}
          <div className="p-4 flex flex-col gap-1.5">
            {isLatest && !streamed ? (
              <StreamingMessage
                content={message.content}
                onComplete={() => setStreamed(true)}
              />
            ) : (
        ...
        {/* Timestamp */}
        <p className="text-[10px] text-slate-700 mt-1 ml-2">
