...
const AIExecutiveSummary: React.FC<AIExecutiveSummaryProps> = ({
  ...
  if (isLoading) {
    return (
      <div className="rounded-2xl border border-white/10 bg-slate-900/60 backdrop-blur-xl p-6 flex flex-col gap-4">
    ...
    >
      ...
      <div className="p-6 flex flex-col gap-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          {gaps.length > 0 && (
            <div className="flex flex-col gap-2">
              <span className="flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-widest text-amber-500">
        ...
        {explanation.reasoning && (
          <details className="group">
            <summary className="cursor-pointer text-[10px] font-bold uppercase tracking-widest text-slate-500 hover:text-slate-300 transition-colors select-none list-none flex items-center gap-1.5">
              <ChevronRight
                size={11}
            ...
            <pre className="mt-3 p-4 rounded-xl bg-slate-950 text-slate-400 text-[11px] font-mono overflow-x-auto whitespace-pre-wrap leading-relaxed border border-slate-800/60">
              {explanation.reasoning}
            </pre>
          </details>
        )}
